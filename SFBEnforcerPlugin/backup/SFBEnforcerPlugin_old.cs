...
namespace SFBEnforcerPlugin
{
    public class SFBEnforcerEEPlugin : RouteAgent.Plugin.INLRouteAgentPluginEntry
    {
        ...
        private const int OPERATION_SUCCEED = 0;
        private const int OPERATION_PROCESSING = 4;
        private const int OPERATION_NoManualClassify = 5;
        ...
        public void PreEvaluation(MailItem obMailItem, RouteAgent.Common.EmailEvalInfoManage obEmailEvalInfoMgr, SmtpServer Server)
        {
            ...
            try
            {
                GetQueryResult(obMailItem, lisRecipients, Sender, meetUrl,Server);
            }
            catch (Exception ex)
            {
        ...
        private void SendDataForResponse(PolicyAssistantSoapClient policyAssistantSoapClient, MailItem obMailItem, string queryIndetify, int waitTime, int processTime, int maxWaitTime, DateTime startTime, SmtpServer Server)
        {
            ...
            processTime++;
            if (result.ResultCode == OPERATION_SUCCEED)
            {
                DenyRecipients(obMailItem, result, Server);
            }
            else if (result.ResultCode == OPERATION_PROCESSING)
            {
        ...
        private void GetQueryResult(MailItem obMailItem, List<string> lisRecipients, EmailRecipient Sender, string meetUrl, SmtpServer Server)
        {
            ...
            else if (result.ResultCode == OPERATION_NoManualClassify)
            {
