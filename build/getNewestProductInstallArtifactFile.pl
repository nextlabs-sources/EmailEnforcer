...
#
# DESCRIPTION
#	This script finds an install artifact file (*-install.zip) of the latest build under
#	the specified product folder. The full path of the install artifact file is saved
#	in a Bash script.
#
...
#		- 26PS_perf
#		- 81PS_officepep
#	3. Install artifact zip file always ends with -install.zip
#		For example:
#		- fate-6.0.0.0-151-release-20120607-install.zip
#		- fate-6.0.0.509-81PS_officepep-pcv-20120607-install.zip
#		- spe-6.0.0.511-25PS_perf-pcv-20120602-install.zip
#		- destiny-6.0.0.703-15PS-archon-201211142106-install.zip
#
...
print "NextLabs Newest Install Artifact File Locator v0.1\n";
...
sub printUsage
{
	print "usage: getNewestProductInstallArtifactFile.pl [--verbose] --startpath=<path> --outfile=<file>\n";
	print "         --varname=<name>\n";
	print "       getNewestProductInstallArtifactFile.pl <-h|--help>\n";
...
# -----------------------------------------------------------------------------
# Find product install artifact file (*-install.zip)
#
# Description
#	The path passed in should look like this:
...
#
# Return Value:
#	Return a value > 0 if a file name matches *-install.zip. Return 0 if there is no match.
#	Install artifact file name is returned in the $refBuf parameter.

sub getProductInstallArtifactFile()
{
	my	($path, $refBuf) = @_;
...
if (!&getProductInstallArtifactFile($path, \$fileName))
{
	exit 1;
}
...
print "Install artifact file = $file\n";
