    ...
    static public class XMLTools
    {
        public static T LoadFromXml<T>(string filePath)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    CSLogger.OutputLog(LogLevel.Error, "Exception during LoadFromXml file:[{0}]", new object[] { filePath }, ex);
                }
            }
            return result;
        }
        ...
        static public string XmlDocmentToString(XmlDocument xmlDoc)
        {
            ...
            using (var stringWriter = new StringWriter())
            {
                using (var xmlTextWriter = XmlWriter.Create(stringWriter))
                {
                    ...
                    xmlTextWriter.Flush();
                    strRet = stringWriter.GetStringBuilder().ToString();
                }
            }
            return strRet;
        }
        ...
        static public Dictionary<string, string> GetAllSubNodesInfo(XmlNode obParentNode)
        {
            ...
            if (null != obParentNode)
            {
                foreach (XmlNode obSubNode in obParentNode.ChildNodes)
                {
                    ...
                    {
                        CommonTools.AddKeyValuesToDic(dicSubNodesInfo, obSubNode.Name, GetXMLNodeText(obSubNode), false);
                    }
                }
            }
