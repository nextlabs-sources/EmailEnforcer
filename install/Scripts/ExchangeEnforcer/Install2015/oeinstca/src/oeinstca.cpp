...
#define FILENAME_EXCHANGEPEP L"exchangepep.xml"
#define FILENAME_LOGCFG L"logcfg.xml"
#define FILENAME_PLUGIN L"PlugIn.xml"
...
// same logic as above function
UINT __stdcall FindExchangePEPXMLFile(MSIHANDLE hInstall)
{
	WCHAR wstrSourceDir[MAX_PATH + 1] = { 0 };
	WCHAR wstrTemp[MAX_PATH + 1] = { 0 };
	DWORD dwPathBuffer = 0;
	...
	MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: Start checking file ExchangePEP.XML"));
	...
	wcscat_s(wstrTemp, FILENAME_EXCHANGEPEP);
	SetFileAttributes(wstrTemp, FILE_ATTRIBUTE_NORMAL);
	//Clean up temp file first
	DeleteFile(wstrTemp);
	...
	if (bFindFile == FALSE)
	{
		...
		wcscat_s(wstrSourceDir, FILENAME_EXCHANGEPEP);

		if (GetFileAttributes(wstrSourceDir) == INVALID_FILE_ATTRIBUTES && GetLastError() == ERROR_FILE_NOT_FOUND)
		{
			MessageAndLogging(hInstall, TRUE, TEXT("The installer could not find the ExchangePEP.xml file in the MSI folder."));
		}
		else
		{
			MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: Use ExchangePEP.xml from MSI folder"));
			bFindFile = TRUE;
		}
	}
	...
	if (bFindFile == FALSE)
	{
		// Check File in installer folder c:\program files\nextlabs\exchange enforcer\config
		ZeroMemory(wstrSourceDir, sizeof(wstrSourceDir));
		dwPathBuffer = sizeof(wstrSourceDir) / sizeof(WCHAR);
		uiRet = 0;
		uiRet = MsiGetProperty(hInstall, TEXT("INSTALLDIR"), wstrSourceDir, &dwPathBuffer);
		if (uiRet == ERROR_SUCCESS)
		{
			...
			wcscat_s(wstrSourceDir, L"Exchange Enforcer\\config\\");
			wcscat_s(wstrSourceDir, FILENAME_EXCHANGEPEP);
			MessageAndLogging(hInstall, TRUE, wstrSourceDir);
			...
			if ((GetFileAttributes(wstrSourceDir) == INVALID_FILE_ATTRIBUTES))
			{
				MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: can't find ExchangePEP.xml from Program Files\\nextlabs\\Exchange Enforcer\\config folder"));
			}
			else
			{
				MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: Use ExchangePEP.xml from existing Program Files\\nextlabs\\Exchange Enforcer\\config folder"));
				bFindFile = TRUE;
			}
		}
	}
	...
	if (bFindFile == FALSE)
	{
		MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: can't find ExchangePEP.xml in both MSI folder and existing exchange enforcer folder. Let's use default one."));
		return ERROR_INSTALL_FAILURE;
	}
	...
	MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: Checking file exchangepep.xml done.  Status: Good."));

	return ERROR_SUCCESS;
}

UINT __stdcall FindLogCFGXMLFile(MSIHANDLE hInstall)
{
	WCHAR wstrSourceDir[MAX_PATH + 1] = { 0 };
	WCHAR wstrTemp[MAX_PATH + 1] = { 0 };
	DWORD dwPathBuffer = 0;
	...
	MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: Start checking file logcfg.XML"));
	...
	wcscat_s(wstrTemp, FILENAME_LOGCFG);
	SetFileAttributes(wstrTemp, FILE_ATTRIBUTE_NORMAL);
	//Clean up temp file first
	DeleteFile(wstrTemp);
	...
	if (bFindFile == FALSE)
	{
		...
		wcscat_s(wstrSourceDir, FILENAME_LOGCFG);

		if (GetFileAttributes(wstrSourceDir) == INVALID_FILE_ATTRIBUTES && GetLastError() == ERROR_FILE_NOT_FOUND)
		{
			MessageAndLogging(hInstall, TRUE, TEXT("The installer could not find the logcfg.xml file in the MSI folder."));
		}
		else
		{
			MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: Use logcfg.xml from MSI folder"));
			bFindFile = TRUE;
		}
	}
	...
	if (bFindFile == FALSE)
	{
		...
		if (uiRet == ERROR_SUCCESS)
		{
			...
			if ((GetFileAttributes(wstrSourceDir) == INVALID_FILE_ATTRIBUTES))
			{
				MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: can't find logcfg.xml from Program Files\\nextlabs\\Exchange Enforcer\\config folder"));
			}
			else
			{
				MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: Use logcfg.xml from existing Program Files\\nextlabs\\Exchange Enforcer\\config folder"));
				bFindFile = TRUE;
			}
		}
	}
	...
	if (bFindFile == FALSE)
	{
		MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: can't find logcfg.xml in both MSI folder and existing exchange enforcer folder. Let's use default one."));
		return ERROR_INSTALL_FAILURE;
	}
	...
	MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: Checking file logcfg.xml done.  Status: Good."));

	return ERROR_SUCCESS;
}

UINT __stdcall FindPluginXMLFile(MSIHANDLE hInstall)
{
	WCHAR wstrSourceDir[MAX_PATH + 1] = { 0 };
	WCHAR wstrTemp[MAX_PATH + 1] = { 0 };
	DWORD dwPathBuffer = 0;
	...
	MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: Start checking file Plugin.XML"));
	...
	wcscat_s(wstrTemp, FILENAME_PLUGIN);
	SetFileAttributes(wstrTemp, FILE_ATTRIBUTE_NORMAL);
	//Clean up temp file first
	DeleteFile(wstrTemp);
	...
	if (bFindFile == FALSE)
	{
		...
		wcscat_s(wstrSourceDir, FILENAME_PLUGIN);

		if (GetFileAttributes(wstrSourceDir) == INVALID_FILE_ATTRIBUTES && GetLastError() == ERROR_FILE_NOT_FOUND)
		{
			MessageAndLogging(hInstall, TRUE, TEXT("The installer could not find the Plugin.xml file in the MSI folder."));
		}
		else
		{
			MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: Use Plugin.xml from MSI folder"));
			bFindFile = TRUE;
		}
	}
	...
	if (bFindFile == FALSE)
	{
		...
		if (uiRet == ERROR_SUCCESS)
		{
			...
			if ((GetFileAttributes(wstrSourceDir) == INVALID_FILE_ATTRIBUTES))
			{
				MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: can't find Plugin.xml from Program Files\\nextlabs\\Exchange Enforcer\\config folder"));
			}
			else
			{
				MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: Use Plugin.xml from existing Program Files\\nextlabs\\Exchange Enforcer\\config folder"));
				bFindFile = TRUE;
			}
		}
	}
	...
	if (bFindFile == FALSE)
	{
		MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: can't find Plugin.xml in both MSI folder and existing exchange enforcer folder. Let's use default one."));
		return ERROR_INSTALL_FAILURE;
	}
	...
	MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: Checking file Plugin.xml done.  Status: Good."));
...
// call in defered execution in system context
UINT __stdcall CopyExchangePEPXMLFile(MSIHANDLE hInstall) //run in defered execution
{
	WCHAR wstrSourceDir[MAX_PATH + 1] = { 0 };
	WCHAR wstrInstallDir[MAX_PATH + 1] = { 0 };
	DWORD dwPathBuffer = 0;
	...
	MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: Start copy ExchangePEP.XML."));
	//get current Installdir from MSI
	dwPathBuffer = sizeof(wstrInstallDir) / sizeof(WCHAR);
	if (ERROR_SUCCESS != MsiGetProperty(hInstall, TEXT("CustomActionData"), wstrInstallDir, &dwPathBuffer))
	{
	...
	wcscat_s(wstrInstallDir, L"Exchange Enforcer\\config\\");

	if ((GetFileAttributes(wstrInstallDir) == INVALID_FILE_ATTRIBUTES))
	{
		MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: we guess exchange enforcer is not installed, wont copy anything"));
		return ERROR_INSTALL_FAILURE;
	}

	wcscat_s(wstrInstallDir, FILENAME_EXCHANGEPEP);
	...
	{
		dwErrorCode = GetLastError();
		swprintf_s(wstrMsg, 128, L"Copy ExchangePEP.xml file failed. Error Code: %d", dwErrorCode);
	...
	//Clean up file
	DeleteFile(wstrSourceDir);
	MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: Copy file ExchangePEP.xml success."));

	return ERROR_SUCCESS;
}

// call in defered execution in system context
UINT __stdcall CopyLogCFGXMLFile(MSIHANDLE hInstall) //run in defered execution
{
	WCHAR wstrSourceDir[MAX_PATH + 1] = { 0 };
	WCHAR wstrInstallDir[MAX_PATH + 1] = { 0 };
	DWORD dwPathBuffer = 0;
	...
	MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: Start copy logcfg.XML."));
	//get current Installdir from MSI
	dwPathBuffer = sizeof(wstrInstallDir) / sizeof(WCHAR);
	if (ERROR_SUCCESS != MsiGetProperty(hInstall, TEXT("CustomActionData"), wstrInstallDir, &dwPathBuffer))
	{
	...
	wcscat_s(wstrInstallDir, FILENAME_LOGCFG);
	...
	{
		dwErrorCode = GetLastError();
		swprintf_s(wstrMsg, 128, L"Copy logcfg.xml file failed. Error Code: %d", dwErrorCode);
	...
	//Clean up file
	DeleteFile(wstrSourceDir);
	MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: Copy file logcfg.xml success."));

	return ERROR_SUCCESS;
}
// call in defered execution in system context
UINT __stdcall CopyPluginXMLFile(MSIHANDLE hInstall) //run in defered execution
{
	WCHAR wstrSourceDir[MAX_PATH + 1] = { 0 };
	WCHAR wstrInstallDir[MAX_PATH + 1] = { 0 };
	DWORD dwPathBuffer = 0;
	...
	MessageAndLogging(hInstall, TRUE, TEXT("******** NXPCLOG: Start copy Plugin.XML."));
	//get current Installdir from MSI
	dwPathBuffer = sizeof(wstrInstallDir) / sizeof(WCHAR);
	if (ERROR_SUCCESS != MsiGetProperty(hInstall, TEXT("CustomActionData"), wstrInstallDir, &dwPathBuffer))
	{
	...
	wcscat_s(wstrInstallDir, FILENAME_PLUGIN);
	...
	{
		dwErrorCode = GetLastError();
		swprintf_s(wstrMsg, 128, L"Copy Plugin.xml file failed. Error Code: %d", dwErrorCode);
...
UINT __stdcall FindConfigFile(MSIHANDLE hInstall )
{
	...
	FindExchangePEPXMLFile(hInstall);
	FindLogCFGXMLFile(hInstall);
	FindPluginXMLFile(hInstall);
...
UINT __stdcall CopyConfigFile(MSIHANDLE hInstall ) //run in defered execution
{
	...
	CopyExchangePEPXMLFile(hInstall);
	CopyPluginXMLFile(hInstall);
	CopyLogCFGXMLFile(hInstall);
...
UINT __stdcall RefreshEnv(MSIHANDLE hInstall)
{
	...
	SendMessageTimeout(HWND_BROADCAST, WM_SETTINGCHANGE, 0, (LPARAM)TEXT("Environment"), SMTO_ABORTIFHUNG, 20000, &dwReturnValue);
...
UINT __stdcall ResetDelayDelete(MSIHANDLE hInstall)
{
	...
	__try
	{
		...
		{
			bFound = false;
			if (NULL != _tcsstr(lpValue, wstrInstallDir))
			{
				bFound = true;
			}
			if (NULL != _tcsstr(lpValue, TEXT("celog")))
			{
				bFound = true;
			}
			if (NULL != _tcsstr(lpValue, TEXT("nlinjection.sys")))
			{
				bFound = true;
			}
			if (NULL != _tcsstr(lpValue, TEXT("nl_tamper.sys")))
			{
				bFound = true;
			}
			if (NULL != _tcsstr(lpValue, TEXT("nlcc.sys")))
			{
				bFound = true;
			}
